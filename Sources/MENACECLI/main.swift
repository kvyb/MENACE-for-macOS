import AppKit
import Foundation
import MENACEBuilder

private let help = """
MENACE for macOS \(MENACEBuilder.version)

Build a macOS app for your own Windows or Steam copy of MENACE.

USAGE
  menace-macos build /absolute/path/MENACE.rar
  menace-macos build                         # opens a Finder picker
  menace-macos steam                         # Steam owns and installs the game
  menace-macos doctor

OPTIONS
  --output PATH          Output folder (default: ~/Desktop/MENACE macOS)
  --cache PATH           Dependency cache folder
  --dependencies-dir P   Use matching dependency archives from this folder
  --no-dmg               Build only MENACE.app
...
INPUTS
  Local: RAR, 7Z, ZIP, an extracted folder, or Menace.exe.
...
private func chooseSource() -> URL? {
    ...
    panel.title = "Choose your Windows copy of MENACE"
    panel.message = "Select a RAR, 7Z, ZIP, Menace.exe, or extracted game folder."
    panel.prompt = "Choose"
...
private func parseBuild(_ rawArguments: [String], steam: Bool = false) throws -> BuildOptions? {
    ...
    var output = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Desktop/MENACE macOS", isDirectory: true)
    var cache = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Caches/MENACEForMacOS", isDirectory: true)
    ...
    while index < rawArguments.count {
        ...
        default:
            ...
            guard !steam else {
                throw BuilderFailure.message("Steam mode does not take a game path. Use: menace-macos steam")
            }
...
do {
    ...
    if arguments.isEmpty || arguments[0] == "-h" || arguments[0] == "--help" || arguments[0] == "help" {
        ...
        if let options = try parseBuild(arguments, steam: steam) {
            _ = try MENACEBuilder(options: options).build()
        }
    }
} catch {
